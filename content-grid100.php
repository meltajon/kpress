<article id="post-<?php the_ID(); ?>">
	<div class="block">
		...
		<?php if (get_post_meta($post->ID, 'embed_code', true) != "") { 
			...
			<?php } ?>
		<?php } ?>
			<h1 class="module-title title pad-x"><a class="title-link" href="<?php the_permalink(); ?>" title="Full Post: <?php the_title_attribute('echo=0'); ?>"><?php the_title(); ?></a></h1>
		</header><!-- .module-header -->
		<?php if ( is_search() ) : // Only display Excerpts for Search ?>
		<div class="module-excerpt pad-x">
			<?php the_excerpt(); ?>
		</div><!-- .module-excerpt -->
		<?php else : ?>
		<div class="module-body pad-x">
			<?php the_content('Continue reading <span class="meta-nav">&rarr;</span>'); ?>
		</div><!-- .module-body -->
		<footer class="module-footer pad-x">
